...
export default function AddressForm({ selectedEmployee , formMode }) {
  ...
  // const handleSubmit = async (e) => {
    // e.preventDefault();
    // if (!branchId || !firstName || !lastName || !password || !tel || !email) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!branchId || !firstName || !lastName || !password || !tel || !email || !role) {
      setError("Please fill in all required fields.");
      return;
    }
    if (formMode === "Add") {
      ...
      try {
        ...
        const response = await EmployeeService.editEmployee(selectedEmployee.id,updatedData);
        if(response.status == 200) {
          swal({
    ...
    <React.Fragment>
      <Box component="form" onSubmit={handleSubmit}>
        <Container>
          ...
          >
            {formMode === "Add" ? "เพิ่ม" : "อัพเดท"} {/* Toggle button label based on form mode */}
          </Button>
        </Container>
      </Box>
    </React.Fragment>
