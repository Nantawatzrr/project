...
export default function AddressForm({selectedpatients , formMode}) {
  ...
  const handlesubmit = async (e) => {
    e.preventDefault();
    if (!branchId || !firstName || !lastName || !peopleCode || !recentIllness || !consciousness || !patientHistory || !receivedDate || !lastReceived) {
    ...
    if(formMode === "Add"){
      ...
      try{
        ...
        const response = await PatientsService.editPatients(selectedpatients.id,updatedData);
        if(response.status == 200){
          swal({
